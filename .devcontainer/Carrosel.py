...
def carregar_dados():
    ...
    for col in ['Itens Sep', 'Horas', 'Itens/Hora', 'Jornada Líq.', 'Ressup.', 'Ressup. Eq.', 'Mov. Horizontal', 'Mov. Vert.']:
        if col in df.columns:
            ...
            if col == 'Jornada Líq.':
                df.loc[(df[col] > 0) & (df[col] <= 1.1), col] = df[col] * 100
    ...
    if not combinacao_valida:
        ...
        for i in range(3):
            with colunas_ui[i]:
                if i < len(inds_f):
                    ind = inds_f[i]
                    df_graf = df_tela[df_tela[ind] > 0].copy()
                    
                    st.markdown(f"<h3 style='text-align: center; color: #333;'>{ind}</h3>", unsafe_allow_html=True)
                    
                    if not df_graf.empty:
                        df_graf = df_graf.sort_values(by=ind, ascending=False).head(15).sort_values(by=ind, ascending=True)
                        ...
                        # --- SOLUÇÃO 1: Fim das Barras Gordas e Magras ---
                        # Obriga o gráfico a alinhar todos os nomes uniformemente
                        ...
                        fig.update_layout(
                            ...
                            bargap=0.4 # Mantém a grossura das barras sempre idêntica
                        )
                        ...
                        fig.update_traces(
                            ...
                            cliponaxis=False # Impede que os números fiquem cortados na borda do ecrã
                        )
                        
                        # --- SOLUÇÃO 2: Fim dos Gráficos Fantasmas ---
                        # O parâmetro 'key' cria uma identidade única e apaga a memória antiga
                        st.plotly_chart(fig, use_container_width=True, key=f"plot_{f_atual}_{ind}")
                    else:
                        st.info("Sem dados no momento.")
                else:
                    # Limpa completamente a coluna se não houver 3 indicadores
                    st.empty()
