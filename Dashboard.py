            ...
            if metas_cargo:
                ...
                with col_tabela:
                    ...
                    for col in df_tabela_mini.columns:
                        if col inFala, Gui! Excelente feedback. Você pegou exatamente o ponto fraco do layout vibrante que montamos: o contraste. Branco sobre o amarelo neon "acende" demais e borra a leitura. Design Logístico precisa de contraste máximo!

Fiz os ajustes cirúrgicos exatamente como você pediu:

### 🛠️ O que eu mudei no código:

1.  **Lógica Dinâmica de Cor do Texto (Plotly):** Criei uma lista de cores inteligente para as porcentagens dentro das barras. O sistema agora verifica a porcentagem (`Real`) de cada barra:
    * **Se for Amarelo Vivo** (Meta 50% Parcial): O texto dentro dela vira **`black`** (preto), dando contraste total.
    * **Para todas as outras cores** (Azul, Verde, Vermelho vibrantes): O texto continua **`white`** (branco), pois elas contrastam bem com o claro.
2.  **Negritos Reforçados (Subtítulos):** Subi a "Grossura" (font-weight) dos subtítulos dos cards superiores para ficarem em **Negrito Máximo (900)**, combinando com o título principal.
3.  **Refino Visual do Gráfico:** Adicionei uma borda branca super fina (`1px`) em todas as barras do gráfico Plotly. Isso ajuda a "cortar" a barra em relação ao fundo preto, dando um acabamento mais profissional e evitando que as cores vibrantes se "misturem" com o escuro.

Pode dar o **`Ctrl + A`** e colar essa nova versão na TV. Agora o amarelo está tão fácil de ler quanto o verde!
...
@st.cache_data(ttl=600) 
def carregar_dados():
    link_csv = "[https://docs.google.com/spreadsheets/d/e/2PACX-1vSDct-pz8fIwAXk-GX5Zcd-dknBBq4Dy4B0pbz6W8vDIvwjdWE2_e7ZQfefMRQcKG4-tvqdQR1Z4zMp/pub?output=csv](https://docs.google.com/spreadsheets/d/e/2PACX-1vSDct-pz8fIwAXk-GX5Zcd-dknBBq4Dy4B0pbz6W8vDIvwjdWE2_e7ZQfefMRQcKG4-tvqdQR1Z4zMp/pub?output=csv)"
