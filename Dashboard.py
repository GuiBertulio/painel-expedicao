...
# =============================================================================
# 🧠 2. CARREGAMENTO E TRATAMENTO DOS DADOS GERAIS
# =============================================================================
...
def carregar_diarios():
    # Cria gavetas (dicionário) para guardar as 3 bases de dados diferentes
        ...
        if not dados_pessoa.empty:
            ...
            if grafico_dados:
                ...
                with col_tabela:
                    ...
                    # =============================================================================
                    # 📅 DRILL-DOWN DIÁRIO (Separadores, Operadores e Conferentes)
                    # =============================================================================
                    # Descobre qual base de dados o robô deve puxar e qual a etiqueta da caixa azul
                    usa_diario = False
                    titulo_movimento = ""
                    ...
                    if "SEPARADOR" in cargo_p:
                        usa_diario = True
                        titulo_movimento = "Itens Separados"
                        df_uso_diario = df_diario
                    elif "OPERADOR" in cargo_p:
                        usa_diario = True
                        titulo_movimento = "Mov. Horizontal"
                    ...
                    if usa_diario:
                        if not df_uso_diario.empty:
                            ...
                            if not df_pessoa_diario.empty:
                                ...
                                # Caça as datas brutas no cabeçalho do arquivo Diário correspondente (Ignorando outras palavras)
                                for c in df_uso_diario.columns:
                                    c_str = str(c).strip()
                                    if any(char.isdigit() for char in c_str) and ('/' in c_str or '-' in c_str) and 'Inicio' not in c_str and 'Horas' not in c_str and 'Itens' not in c_str and 'JL' not in c_str and 'Mov' not in c_str:
                                ...
                                if cols_datas_reais:
                                    ...
                                    # Cruza para achar a coluna da planilha selecionada
                                    ...
                                    # Puxa o Volume (Seja Item ou Movimentação), Horas e Velocidade
                                    ...
                                    # Se for Operador/Conferente, escreve "Mov/Hora" ao invés de "Itens/Hora"
                                    titulo_velocidade = "⚡ Mov./Hora" if "Mov." in titulo_movimento else "⚡ Itens/Hora"
                                    c2.metric(titulo_velocidade, val_itens_hora)
                                    
                                    c3.metric("🎯 JL", jl_display)
                                    
                                    # Banner Gigante em Azul para a Métrica Principal do dia
                                    st.markdown(f"<div style='background-color: rgba(59, 130, 246, 0.1); padding: 15px; border-radius: 10px; border-left: 5px solid {C_AZUL}; margin-top: 15px; margin-bottom: 15px;'><h4 style='margin:0; color: #888;'>{titulo_movimento}</h4><h2 style='margin:0; color: {C_AZUL};'>{val_itens}</h2></div>", unsafe_allow_html=True)
